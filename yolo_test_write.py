...
txt_result_path = 'mAP/input/detection-results'
...
for i in os.listdir(image_result_path):
    path_file = os.path.join(image_result_path,i)  
    if os.path.isfile(path_file):
        os.remove(path_file)
for i in os.listdir(txt_result_path):
    path_file = os.path.join(txt_result_path,i)  
    if os.path.isfile(path_file):
        os.remove(path_file)
#创建一个记录检测结果的文件
...
#                    outline=self.colors[c])
#
#            draw.text(text_origin, label, fill=self.colors[c], font=font)
#            del draw
        end = timer()
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(path): 
        ...
        file.write('\n')
        #r_image.show() 显示检测结果
        image_save_path = image_result_path +'/'+ portion[1]        
